...
int maxE = 100;
int maxC = s * maxE;
int C[s][s];     //Ma trận chi phí
int X[s + 1];       //X đểthửcác khảnăng, BestWay đểghi nhận nghiệm
int T[s];           //T[i] đểlưu chi phí đi từX[1] đến X[i]
int BestWay[s + 1]; //, BestWay để ghi nhận nghiệm
bool FREE[s];       //Free để đánh dấu, Free[i]= True nếu chưa đi qua tp i
        ...
        if (FREE[j]) //Nếu gặp thành phố chưa đi qua
        {
            ...
            if (T[i] < minSpending)           //Hiển nhiên nếu có điều này thì C[x[i - 1], j] < +∞ rồi
            {
                ...
                if (i == N)
                {
                    ...
                    tim(i + 1); //  Tìm các khả năng chọn x[i+1]
